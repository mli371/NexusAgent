...
stage (query_embedding -> vector_search, in parallel with full_text_search)
stage (rrf_fusion -> reranking -> parent_expansion -> context_building)
...
On hit, query_embedding, vector_search, full_text_search, rrf_fusion, reranking, parent_expansion and context_building emit `skipped` with debug summary `reason=cache_reuse`, duration 0 (not measured execution). cache_lookup's debug summary includes status and reason. No cached stage events are replayed. Capabilities advertise `retrievalCacheMode=versioned_context`, or `bypassed` when disabled; this is configuration, not a Redis connectivity probe. Changes to documents during a request can still return `DOCUMENT_CHANGED` rather than automatically retrying a potentially billed answer.
