...
- Optional bounded semantic context caching follows an exact miss: reuse the existing question embedding, compare up to 100 same-scope source vectors in Java, apply a configurable cosine threshold and lexical guards, then revalidate evidence. A semantic miss passes that vector into normal retrieval without a second embedding call. Only fresh retrieval seeds the cache; semantic hits do not create aliases, reseed, or extend TTL. The UI distinguishes exact/semantic hits and labels historical scores. Default threshold 0.96 is uncalibrated, not an accuracy guarantee. See [semantic cache learning note](docs/learning/semantic-context-cache.md).
- Page-local follow-ups carry at most three bounded, answered turns. After access/readiness preflight and before either cache, one structured model call resolves a standalone question or requests clarification. The first question skips this call. Resolved questions drive both caches, retrieval and answer generation; historical answers are reference hints, not evidence or Redis conversation memory. No paid retry is automatic. See the [Chinese learning note](docs/learning/page-follow-up-resolution.md) and [implementation review](docs/review/page-follow-up-resolution.md).
- Coverage-first context allocation keeps the existing 4000-character default: reserve complete child evidence before sharing remaining space across child-centered parent windows. A short parent returns unused space to other parents. The learning UI adds child selection, per-parent allocations and explicit budget exclusions; evidence count and answer citation count are distinct. New cache policy versions prevent reuse of the former greedy allocation. This preserves selected evidence, not guaranteed coverage of every document/year or every answer claim. See the [Chinese learning note](docs/learning/coverage-first-context.md) and [review](docs/review/coverage-first-context.md).
...
Open **http://127.0.0.1:5173/**. Use Node 22.22.2 or newer. Restart an older backend to load the new query scopes/capabilities, including `pageFollowUpSupported=true`. This phase needs no new database migration. Configuration readiness is not proof of working API credentials.

The browser uses demo tenant/actor headers, not login. Upload a TXT/Markdown fixture from **Documents and preparation**, then use the processing view to propose and manually approve CHUNK/EMBED_MISSING. Model-mismatched vectors require a separately confirmed rebuild; queries never migrate them automatically. QA keeps at most 12 displayed turns in page memory and sends the latest three completed, answered turns in the same scope (each question up to 2000 characters, answer excerpt up to 1000). Refresh, clear, identity/view changes and changes to scope or selected document set discard history and rotate session ID. Adjusting topK/budget does not. A POST stream is never automatically resubmitted after disconnection. Consent covers the current question, bounded history and retrieved evidence, including the extra resolution-model call when history is present. Ambiguity returns a clarification without retrieval or answer generation; `NEXUS_QUERY_REWRITE_TIMEOUT` defaults to 20s.
