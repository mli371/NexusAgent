...
export function QueryWorkbench({ identity, onIdentity, onManage, onBusy }: {
  ...
  const [consent, setConsent] = useState(false), [sessionId] = useState(() => crypto.randomUUID());
  ...
  const send = () => {
    ...
    void query.submit({ sessionId, question: question.trim(), scope, documentIds: scope === "documents" ? documents.map(d => d.id) : [],
      topK, contextBudgetChars: budget, debug: true }, capabilities!.answerModel!);
    setQuestion("");
  };
  ...
  return <div className="qa-page">
    ...
    <div className="notice"><ShieldCheck /><span>Header 演示身份，不是生产认证。问题与检索片段将发送至 OpenAI；请只使用获准的合成资料。</span></div>
    ...
    <div className="qa-toolbar">
      <div className="qa-scope" role="group" aria-label="检索范围">
        <button aria-pressed={scope === "library"} onClick={() => setScope("library")} disabled={query.running}><BookOpen />整个知识库</button>
        <button aria-pressed={scope === "documents"} onClick={() => setScope("documents")} disabled={query.running}><Files />指定文档{documents.length > 0 ? ` · ${documents.length}` : ""}</button>
      </div>
    ...
    {!ready && <div className="warning page-error" role="status">{connectionError || (capabilities
      ? "后端未开启双范围真实问答。需要更新后端，并配置 NEXUS_ANSWER_PROVIDER=openai 与 NEXUS_EMBEDDINGS_PROVIDER=openai；不会自动退回模板。"
      : "正在读取后端模型配置…")}</div>}
    ...
    <main className="workspace qa-workspace">
      <section className={`panel conversation ${view === "conversation" ? "mobile-active" : ""}`}>
        ...
        <div className="chat-body qa-chat" aria-live="polite">
          {query.turns.length === 0 ? <Empty><BookOpen /><h3>从知识库提问</h3><p>{scope === "library" ? "范围：当前身份可访问且向量就绪的文档" : `范围：已选 ${documents.length} 份文档`}</p></Empty>
            : query.turns.map((turn, index) => <article className={`qa-turn ${query.current?.traceId === turn.traceId ? "selected" : ""}`} key={turn.traceId}>
              ...
              {turn.response ? <>
                <p className="speaker qa-answer-label">{turn.response.answerStatus === "answered" ? "回答" : turn.response.answerStatus === "refused" ? "模型拒答" : "证据不足"} · {turn.response.answerModel}</p>
                ...
                <div className="qa-scope-result">检索 {turn.response.scope.searchedDocumentCount} / {turn.response.scope.accessibleDocumentCount} 份文档 · {{ hit: "上下文缓存命中", miss: "上下文缓存未命中", bypassed: "缓存已绕过" }[turn.response.retrievalCacheStatus]}</div>
        ...
        <form className="composer qa-composer" onSubmit={e => { e.preventDefault(); send(); }}>
          ...
          <label className="qa-consent"><input type="checkbox" checked={consent} disabled={query.running} onChange={e => setConsent(e.target.checked)} />允许外发本次页面的问题与检索片段，可能产生 API 用量</label>
        </form>
      </section>
    ...
    <footer className="bottom"><span>单轮问答 · 本页最多保留 12 条 · 刷新或离开视图清空</span><span>启发式重排 · 字符预算 · 引用关联不等于事实性验证</span></footer>
    {showDocuments && <DocumentPanel api={api} selected={documents} queryMode
      onSelect={docs => { setDocuments(docs); setScope("documents"); }} onClose={() => setShowDocuments(false)}
