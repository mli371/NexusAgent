...
export function QueryFlow({ turn, selection, onSelect, active }: {
  ...
  const node = (id: StageId) => {
    ...
    const cacheStatus = id === "cache_lookup" && stage?.summary?.cacheStatus;
    const title = reused ? "缓存复用" : cacheStatus === "hit" ? "命中" : cacheStatus === "miss" ? "未命中"
    ...
    return <button className={`qa-node ${status} ${interrupted ? "interrupted" : ""}`} data-stage={id} data-state={status}
      ...
      <span className="qa-node-state">{title}{stage && status !== "running" && <small>{reused ? "未执行" : `${stage.durationMs} ms`}</small>}</span>
    </button>;
  };
  ...
  return <section className={`panel flow qa-flow ${active ? "mobile-active" : ""}`}>
    ...
    <div className="graph qa-graph">
      <div className="qa-lane">
        {series(["access_check", "embedding_readiness", "cache_lookup"])}
        <div className="qa-parallel" aria-label="并行检索分支">
          <div>{series(["query_embedding", "vector_search"])}</div>
          <div>{series(["full_text_search"])}</div>
        </div>
        {series(["rrf_fusion", "reranking", "parent_expansion", "context_building", "answer_generation", "citation_validation"])}
        {turn && turn.status !== "running" && <p className={`qa-terminal ${turn.status}`}>
          {turn.status === "completed" ? "完成 · 最终权限复查通过" : turn.status === "failed" ? "请求失败 · 未返回已验证答案" : "已停止等待 · 远端结果未确认"}
        </p>}
      </div>
    </div>
